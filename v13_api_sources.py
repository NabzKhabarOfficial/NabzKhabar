...
def collect_api_candidates():
    """Collect only the two explicitly enabled API sources for NABZ-V13.

    Currents and World News API are enabled here. NewsData/GNews remain
    disabled because their free tiers do not satisfy the 30-minute freshness
    contract; GDELT is intentionally not activated to avoid extra network
    load/rate-limit noise.
    """
    out = []
    for name, fn in (
        ("Currents", _currents),
